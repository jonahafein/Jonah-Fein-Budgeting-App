...
if continue_on_step4 == False:
    ...
    dummy_take_home = utils.calculate_monthly_take_home(single = single, annual_income = annual_income, annual_bonus = st.session_state.annual_bonus, trad_401k_contributions = 0, standard_deduction = standard_deduction, state_tax_perc = st.session_state.state_tax_perc, local_tax_perc = st.session_state.local_tax_perc, months_worked = st.session_state.months_worked)
    ...
    trad_401k_contributions = st.slider("How much would you like to contribute to your traditional 401k this calendar year? (if you have a roth 401k, we recommend you max that first).", max_value=int(12*(dummy_monthly_margin)), step=1)
    trad_401k_match_annual = 0
    if trad_401k_contributions > 0:
        trad_401k_match_annual = st.slider(f"How much will your employer match of the ${trad_401k_contributions} you put into your traditional 401k this year?", max_value = trad_401k_contributions, step=1)
    st.session_state.trad_401k_match_annual = trad_401k_match_annual
    monthly_take_home = utils.calculate_monthly_take_home(single = single, annual_income = annual_income, annual_bonus = annual_bonus, trad_401k_contributions = trad_401k_contributions, standard_deduction = standard_deduction, state_tax_perc = st.session_state.state_tax_perc, local_tax_perc = st.session_state.local_tax_perc, months_worked = st.session_state.months_worked)
    monthly_margin = utils.calculate_monthly_margin(monthly_take_home = monthly_take_home, expenses_df = st.session_state.expenses_df, trad_401k_contributions = trad_401k_contributions, months_worked = months_worked)
...
if st.session_state.savings < 1000:
    ...
    monthly_take_home = utils.calculate_monthly_take_home(single = single, annual_income = annual_income, annual_bonus = st.session_state.annual_bonus, trad_401k_contributions = 0, standard_deduction = standard_deduction, state_tax_perc = st.session_state.state_tax_perc, local_tax_perc = st.session_state.local_tax_perc, months_worked = st.session_state.months_worked)
    monthly_margin = utils.calculate_monthly_margin(monthly_take_home = monthly_take_home, expenses_df = st.session_state.expenses_df, trad_401k_contributions = trad_401k_contributions, months_worked = months_worked)
...
if continue_on_step4:
    st.write(f"Recall, under the assumption that you put ${trad_401k_contributions:,.2f} annually (which is ${float(trad_401k_contributions/months_worked):,.2f} a month) into a traditional 401k this year, you will have ${monthly_margin:,.2f} left a month. (Note: margin is post traditional 401k contributions).")
    roth_ira_monthly = st.slider("How much will you put into your roth ira a month?", max_value=int(monthly_margin))
    roth_401k_contributions_monthly = st.slider("How much will you put into a roth 401k a month? (ignore if you don't have a roth 401k).", max_value=int(monthly_margin - roth_ira_monthly), step=1)
    roth_401k_match_monthly = 0
    if roth_401k_contributions_monthly > 0:
        roth_401k_match_monthly = st.slider(f"How much will your employer match of the ${roth_401k_contributions_monthly} you put into your roth 401k this month?", max_value = roth_401k_contributions_monthly, step=1)
    ...
    brokerage_contributions_monthly = st.slider("How much will you put into a your brokerage monthly?", max_value=int(monthly_margin - roth_ira_monthly - roth_401k_contributions_monthly), step=1)
